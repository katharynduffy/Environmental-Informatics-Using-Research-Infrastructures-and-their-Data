...
One such example is Sea Surface Temperature from the Advanced Very High Resolution Radiometer (AVHRR) temperature sensing system. Specifying datasets such as this requires finding out the OPeNDAP endpoint (URL) for the dataset, and specifying it as the `url` to webdata (we found this example in the extensive [apdrc.soest.hawaii.edu/data/data.php](http://apdrc.soest.hawaii.edu/data/data.php) catalog):

```{r create-fabric-NOAA, message=FALSE, warning=FALSE}
sst_fabric <- webdata(url='dods://apdrc.soest.hawaii.edu/dods/public_data/satellite_product/AVHRR/avhrr_mon')

# need to set the variable(s) we found from the web catalog. 
variables(sst_fabric) <- c("sst")
# need to set the time range. 
times(sst_fabric) <- c('1990-01-01','1999-12-31')
```
...
wv_var <- query(wv_fabric, "variables")
