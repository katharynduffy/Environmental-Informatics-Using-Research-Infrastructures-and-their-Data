...
Select sites to be converted. All sites are listed below, uncomment the site/sites to be run. Running all sites will take a while!
...
mnth <- c("03", "04")
#define start date 
dateBgnArg <- min(paste0(year,"-", mnth))
dateEndArg <- max(paste0(year,"-", mnth))
...
```{r, eval=FALSE}
  ...
  if(!exists("dateBgnArg") || is.na(dateBgnArg) || is.null(dateBgnArg)){
    dateBgn <- as.Date(metaSite[[idxSite]]$values$GRP_FLUX_MEASUREMENTS[[1]]$FLUX_MEASUREMENTS_DATE_START, "%Y%m%d")
  } else {
    dateBgn <- dateBgnArg
  }#End of checks for missing dateBgn
  
  #Check if dateEnd is defined, if not make it the system date
  if(!exists("dateEndArg") || is.na(dateEndArg) || is.null(dateEndArg)){
...
```{r, eval=FALSE}
  timeOfstUtc <- as.integer(metaSite[[idxSite]]$values$GRP_UTC_OFFSET[[1]]$UTC_OFFSET)
```

Create the method for date sequence based off of the package chosen, the basic package contains monthly files while the expanded package contains daily files
```{r, eval=FALSE}
...
```
  ...
  dataList <- lapply(setDate, function(x) {
    ...
    dataIdx <- tryCatch(som::def.neon.api.get.data.hdf5(site=idxSite,idDpMain=idDpMain,year=year,mnth=mnth,DirDnld=DirDnld,Rm = TRUE), error=function(e) NULL)
    if(!is.null(dataIdx)){
      dataIdx[[idxSite]]$dp0p <- NULL
      dataIdx[[idxSite]]$dp02 <- NULL
      dataIdx[[idxSite]]$dp03 <- NULL
      dataIdx[[idxSite]]$dp01$ucrt <- NULL
      dataIdx[[idxSite]]$dp04$ucrt <- NULL
      dataIdx[[idxSite]]$dp01$data <- lapply(dataIdx[[idxSite]]$dp01$data,FUN=function(var){
        nameTmi <- names(var)
        var <- var[grepl('_30m',nameTmi)]
        return(var)})
      dataIdx[[idxSite]]$dp01$qfqm <- lapply(dataIdx[[idxSite]]$dp01$qfqm,FUN=function(var){
        nameTmi <- names(var)
        var <- var[grepl('_30m',nameTmi)]
        return(var)})
    }
  ...
  #Check if any data is available from the API, if not move to next site
  if(length(dataList) > 0){
    ...
    #Tower top level
    LvlTowr <- grep(pattern = "_30m", names(dataList[[1]][[idxSite]]$dp01$data$co2Turb), value = TRUE)
    ...
    dataListFlux <- lapply(names(dataList), function(x) {
      data.frame(
        "TIMESTAMP_START" = as.POSIXlt(dataList[[x]][[idxSite]]$dp04$data$fluxCo2$turb$timeBgn, format="%Y-%m-%dT%H:%M:%OSZ", tz = "GMT"),
        "TIMESTAMP_END" = as.POSIXlt(dataList[[x]][[idxSite]]$dp04$data$fluxCo2$turb$timeEnd, format="%Y-%m-%dT%H:%M:%OSZ", tz = "GMT"),
        ...
        "FC"= dataList[[x]][[idxSite]]$dp04$data$fluxCo2$turb$flux,
        "SC"= dataList[[x]][[idxSite]]$dp04$data$fluxCo2$stor$flux,
        "NEE"= dataList[[x]][[idxSite]]$dp04$data$fluxCo2$nsae$flux,
        "LE" = dataList[[x]][[idxSite]]$dp04$data$fluxH2o$turb$flux,
        "SLE" = dataList[[x]][[idxSite]]$dp04$data$fluxH2o$stor$flux,
        "USTAR" = dataList[[x]][[idxSite]]$dp04$data$fluxMome$turb$veloFric,
        
        "H" = dataList[[x]][[idxSite]]$dp04$data$fluxTemp$turb$flux,
        "SH" = dataList[[x]][[idxSite]]$dp04$data$fluxTemp$stor$flux,
        "FETCH_90" = dataList[[x]][[idxSite]]$dp04$data$foot$stat$distXaxs90,
        "FETCH_MAX" = dataList[[x]][[idxSite]]$dp04$data$foot$stat$distXaxsMax,
        "V_SIGMA" = dataList[[x]][[idxSite]]$dp04$data$foot$stat$veloYaxsHorSd,
        #"W_SIGMA" = dataList[[x]][[idxSite]]$dp04$data$foot$stat$veloZaxsHorSd,
        "CO2_1_1_1" = dataList[[x]][[idxSite]]$dp01$data$co2Turb[[paste0(LvlTowr,"_30m")]]$rtioMoleDryCo2$mean,
        "H2O_1_1_1" = dataList[[x]][[idxSite]]$dp01$data$h2oTurb[[paste0(LvlTowr,"_30m")]]$rtioMoleDryH2o$mean,
        "qfFinlH2oTurbFrt00Samp" = dataList[[x]][[idxSite]]$dp01$qfqm$h2oTurb[[paste0(LvlTowr,"_30m")]]$frt00Samp$qfFinl,
        "qfH2O_1_1_1" = dataList[[x]][[idxSite]]$dp01$qfqm$h2oTurb[[paste0(LvlTowr,"_30m")]]$rtioMoleDryH2o$qfFinl,
        "qfCO2_1_1_1" = dataList[[x]][[idxSite]]$dp01$qfqm$co2Turb[[paste0(LvlTowr,"_30m")]]$rtioMoleDryCo2$qfFinl,
        "qfSC" = dataList[[x]][[idxSite]]$dp04$qfqm$fluxCo2$stor$qfFinl,
        "qfSLE" = dataList[[x]][[idxSite]]$dp04$qfqm$fluxH2o$stor$qfFinl,
        "qfSH" = dataList[[x]][[idxSite]]$dp04$qfqm$fluxTemp$stor$qfFinl,
        "qfT_SONIC" = dataList[[x]][[idxSite]]$dp01$qfqm$soni[[paste0(LvlTowr,"_30m")]]$tempSoni$qfFinl,
        "qfWS_1_1_1" = dataList[[x]][[idxSite]]$dp01$qfqm$soni[[paste0(LvlTowr,"_30m")]]$veloXaxsYaxsErth$qfFinl,
        rbind.data.frame(lapply(names(idxVerAmfx), function(y) {
          tryCatch({rlog$debug(y)}, error=function(cond){print(y)})
          rpt <- list()
          rpt[[paste0("CO2_1_",idxVerAmfx[y],"_2")]] <- dataList[[x]][[idxSite]]$dp01$data$co2Stor[[paste0(y,"_30m")]]$rtioMoleDryCo2$mean
          
          
          rpt[[paste0("H2O_1_",idxVerAmfx[y],"_2")]] <- dataList[[x]][[idxSite]]$dp01$data$h2oStor[[paste0(y,"_30m")]]$rtioMoleDryH2o$mean
          rpt[[paste0("CO2_1_",idxVerAmfx[y],"_3")]] <- dataList[[x]][[idxSite]]$dp01$data$isoCo2[[paste0(y,"_30m")]]$rtioMoleDryCo2$mean
          
          rpt[[paste0("H2O_1_",idxVerAmfx[y],"_3")]] <- dataList[[x]][[idxSite]]$dp01$data$isoCo2[[paste0(y,"_30m")]]$rtioMoleDryH2o$mean
          rpt[[paste0("qfCO2_1_",idxVerAmfx[y],"_2")]] <- dataList[[x]][[idxSite]]$dp01$qfqm$co2Stor[[paste0(LvlTowr,"_30m")]]$rtioMoleDryCo2$qfFinl
          rpt[[paste0("qfH2O_1_",idxVerAmfx[y],"_2")]] <- dataList[[x]][[idxSite]]$dp01$qfqm$h2oStor[[paste0(LvlTowr,"_30m")]]$rtioMoleDryH2o$qfFinl
          rpt[[paste0("qfCO2_1_",idxVerAmfx[y],"_3")]] <- dataList[[x]][[idxSite]]$dp01$qfqm$isoCo2[[paste0(LvlTowr,"_30m")]]$rtioMoleDryCo2$qfFinl
          rpt[[paste0("qfH2O_1_",idxVerAmfx[y],"_3")]] <- dataList[[x]][[idxSite]]$dp01$qfqm$isoH2o[[paste0(LvlTowr,"_30m")]]$rtioMoleDryH2o$qfFinl
        ...
        "WS_1_1_1" = dataList[[x]][[idxSite]]$dp01$data$soni[[paste0(LvlTowr,"_30m")]]$veloXaxsYaxsErth$mean,
        "WS_MAX_1_1_1" = dataList[[x]][[idxSite]]$dp01$data$soni[[paste0(LvlTowr,"_30m")]]$veloXaxsYaxsErth$max,
        "WD_1_1_1" = dataList[[x]][[idxSite]]$dp01$data$soni[[paste0(LvlTowr,"_30m")]]$angZaxsErth$mean,
        "T_SONIC" = dataList[[x]][[idxSite]]$dp01$data$soni[[paste0(LvlTowr,"_30m")]]$tempSoni$mean,
        "T_SONIC_SIGMA" = base::sqrt(dataList[[x]][[idxSite]]$dp01$data$soni[[paste0(LvlTowr,"_30m")]]$tempSoni$mean)
    ...
    #############################################################################
    #Sequence for Validations
    # valiPos <- seq(from = 17, to = nrow(dataDfFlux), by = 47)
    # # #Remove the validation periods from data
    # dataDfFlux[valiPos, c("FC","LE","CO2", "H2O","H")] <- NaN
    ...
    #Grab all CO2/H2O columns to apply same threshold
  ...
  }#End of if statement to check for available data
