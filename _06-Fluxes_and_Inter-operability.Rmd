...
```{r, eval = FALSE}
  ...
  dataList <- lapply(setDate, function(x) {
    ...
    if(!is.null(dataIdx)){ 
       dataIdx[[site]]$dp0p <- NULL 
       dataIdx[[site]]$dp02 <- NULL 
       dataIdx[[site]]$dp03 <- NULL
       dataIdx[[site]]$dp01$ucrt <- NULL 
       dataIdx[[site]]$dp04$ucrt <- NULL 
       dataIdx[[site]]$dp01$data <- lapply(dataIdx[[site]]$dp01$data,FUN=function(var){ 
         nameTmi <- names(var) 
         var <- var[grepl('_30m',nameTmi)] 
         return(var)})
       dataIdx[[site]]$dp01$qfqm <- lapply(dataIdx[[site]]$dp01$qfqm,FUN=function(var){ 
         nameTmi <- names(var)
         var <- var[grepl('_30m',nameTmi)]
         return(var)})
    }
